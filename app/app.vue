...
/* ── Highlight.js via CSS custom properties ────────────────────────────── */
pre code.hljs {
  display: block;
  overflow-x: auto;
  padding: 1.2em;
...
.hljs-addition {
  color: var(--hljs-tag);
  background: color-mix(in srgb, var(--hljs-tag) 15%, transparent);
}
.hljs-deletion {
  color: var(--hljs-keyword);
  background: color-mix(in srgb, var(--hljs-keyword) 15%, transparent);
}
</style>
