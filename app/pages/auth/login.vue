<template>
  <div class="max-w-sm mx-auto pt-8">
    <div class="text-center mb-8">
      ...
      <h1 class="text-2xl font-bold" style="color: var(--text-primary)">{{ t('auth.welcomeBack') }}</h1>
      <p class="text-sm mt-1" style="color: var(--text-muted)">{{ t('auth.signInSubtitle') }}</p>
    </div>
    ...
    <form @submit.prevent="submit" class="bg-surface-card border border-surface-border rounded-2xl p-6 space-y-4">
      ...
      <p class="text-center text-sm" style="color: var(--text-muted)">
        {{ t('auth.noAccount') }} <NuxtLink to="/auth/register" class="text-accent hover:underline">{{ t('auth.register') }}</NuxtLink>
      </p>
    </form>
  </div>
</template>
